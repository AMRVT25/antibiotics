# This program does ARDL analysis on YEARLY bases for a couple of time series 
...
output_dir = "plots/yearly_price_vs_quantity/"
dir.create(output_dir, showWarnings = FALSE)  # Create directory if it doesn't exist
output_dir = "plots/yearly_price_vs_quantity/not_significant"
dir.create(output_dir, showWarnings = FALSE)  # Create directory if it doesn't exist
output_dir = "plots/yearly_price_vs_quantity/significant"
dir.create(output_dir, showWarnings = FALSE)  # Create directory if it doesn't exist



makePlot = function(product, packsize, df, srGraphInfo, lrGraphInfo, lagorder, isSignificant){
  if (isSignificant) {
    file_name=paste0("plots/yearly_price_vs_quantity/significant/",product,packsize,".png")
  }
  else {
    file_name=paste0("plots/yearly_price_vs_quantity/not_significant/",product,packsize,".png")
  }
  imgWidth = 1600
  imgHeight = 1000
  ...
  plot(df$Date, 
       ...
       main=paste0("Price and Quantity of ", product, ", ", packsize, "\n",
                   "Short run elasticity: ",srGraphInfo, ",   Long run elasiticty: ",lrGraphInfo, "\nLagorder: ", lagorder) ,      
       xlab="Year",
       ylab="SEK/kg",
       xaxt = "n",
...
# Ultrapen and Hippotrim could not be included, since time series is too short. 
...
#for (i in 1 : 1) {
  ...
  for (j in 1:length(sizes)) {
    ...
    if (product == "Ethacilin combined") {
      ...
      df_sales = sqldf(paste0("select * from df_antibiotics where ProductName='",product,"' and PackageSize='",sizes[j],"'"))
    }
    ...
    models <- auto_ardl(ln_TotalKgActiveSubstanceSold ~ ln_AverageAntibioticsRealPricePerKg + ln_headCount + ln_RealAvrakningsprisKrPerKg, data = df,  max_order = 2, selection = "AIC")
    ...
    print("---------------------------------------------")
    print(paste("Short run relationships ",product_main_name, " ", sizes[j]) )
    print(multipliers(ecm_model, type = "sr") )
    print("---------------------------------------------")
    mp = multipliers(ecm_model, type = "sr")
    mp[mp$Term=="ln_AverageAntibioticsRealPricePerKg","Estimate"]
    ...
    if(df_results["Bounds f-test p-value", col]<0.05 & df_results["Bounds t-test p-value", col]<0.05) {
      ...
      print(paste("Long run relationship found",product_main_name, " ", sizes[j]) )
      print(lrmp)
    ...
    if (grepl("\\*\\*", df_results["SR elasticity", col]) | grepl("\\*\\*", df_results["LR elasticity", col]))   {
      ...
      makePlot(product_main_name, sizes[j], df, srGraphInfo, lrGraphInfo, str_lags, TRUE)
    }
    else {
      makePlot(product_main_name, sizes[j], df, srGraphInfo, lrGraphInfo, str_lags, FALSE)
    }
...
write_xlsx(df_results, "results/ARDL_models_result_yearly.xlsx")
write.csv(df_results, "results/ARDL_models_result_yearly.csv", row.names = TRUE)
...
saveWorkbook(wb, "results/ARDL_models_result_style_yearly.xlsx", overwrite = TRUE)
